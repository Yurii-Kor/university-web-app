...
public interface CourseRepository extends JpaRepository<Course, Long> {
    ...
    @Query(
        value = """
            select new ua.foxminded.university.model.persistence.course.projection.CourseCardView(
                c.id,
                c.code,
                c.name,
                c.description,
    ...
    @Query("""
            select new ua.foxminded.university.model.persistence.course.projection.CourseHeaderView(c.id, c.code, c.name)
            from Course c
            where c.id = :id
        """)
