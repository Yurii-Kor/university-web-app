package ua.foxminded.university.service.lesson;
...
import ua.foxminded.university.service.lesson.dto.LessonSelfUpdateDto;
import ua.foxminded.university.service.lesson.exception.ScheduleConflictException;
...
class LessonServiceTest {
	...
	void create_happyPath_success() {
		...
		assertEquals(teacherAlgDs.getId(), temp.getCourse().getTeacher().getId());
		assertEquals(courseALG.getId(), temp.getCourse().getId());
		assertEquals(groupCS.getId(), temp.getGroup().getId());
		assertEquals(ROOM_A, temp.getRoom());
		assertEquals(T13, temp.getStartTime());
		assertEquals(T14, temp.getEndTime());
		assertEquals(LessonType.OTHER, temp.getLessonType());
		assertEquals("note", temp.getDescription());
	}
	...
	void create_nullTeacherId_fails() {
		assertThrows(ConstraintViolationException.class,
				() -> lessonService.create(createDto(null, courseALG.getId(), groupCS.getId(), T10, T12, ROOM_A)));
	}
	...
	void create_nullCourse_fails() {
		assertThrows(ConstraintViolationException.class,
				() -> lessonService.create(createDto(teacherAlgDs.getId(), null, groupCS.getId(), T10, T12, ROOM_A)));
	}
	...
	void create_roomIsTeacherOffice_fails() {
		assertThrows(IllegalStateException.class,
				() -> lessonService.create(createDto(teacherAlgDs
	...
	void updateSelf_changeGroup_happyPath_success() {
		...
		var afterGroup = lessonService.updateSelf(
				patchDto(temp.getId(), teacherAlgDs.getId(), null, groupSE.getId(), null, null, null, null, null));
	...
	void updateSelf_changeCourse_happyPath_success() {
		...
		var afterCourse = lessonService.updateSelf(
				patchDto(temp.getId(), teacherAlgDs.getId(), courseDS.getId(), null, null, null, null, null, null));
	...
	void updateSelf_changeTime_happyPath_success() {
		...
		var afterTime = lessonService
				.updateSelf(patchDto(temp.getId(), teacherAlgDs.getId(), null, null, T14, T15, null, null, null));
	...
	void updateSelf_changeRoom_happyPath_success() {
		...
		var afterRoom = lessonService.updateSelf(
				patchDto(temp.getId(), teacherAlgDs.getId(), null, null, null, null, ROOM_B, null, null));
	...
	void updateSelf_changeTypeAndDescription_happyPath_success() {
		...
		var afterTypeDesc = lessonService.updateSelf(
				patchDto(temp.getId(), teacherAlgDs.getId(), null, null, null, null, null, LessonType.LAB, "   "));
	...
	void updateSelf_noop_ok() {
		var after = lessonService.updateSelf(patchDto(base.getId(),
				teacherAlgDs.getId(),
	...
	void updateSelf_notFound() {
		assertThrows(EntityNotFoundException.class,
				() -> lessonService.updateSelf(patchDto(MISSING_ID,
						teacherAlgDs.getId(),
						null,
						null,
						null,
	...
	void updateSelf_switchToForeignCourse_fails() {
		assertThrows(IllegalStateException.class,
				() -> lessonService.updateSelf(patchDto(base
	...
	void updateSelf_nullPatch_fails() {
		assertThrows(IllegalArgumentException.class, () -> lessonService.updateSelf(null));
	}
	...
	void findByIds_various() {
		assertTrue(lessonService.findByIds(null).isEmpty());
		assertTrue(lessonService.findByIds(List.of()).isEmpty());

		var res = lessonService.findByIds(Arrays.asList(null, base.getId(), base.getId()));
	...
	void deleteByIds_happyPath_success() {
		var created = lessonService
				.create(createDto(teacherAlgDs.getId(), courseALG.getId(), groupCS.getId(), T13, T14, ROOM_A));

		var res = lessonService.deleteByIds(List.of(created.getId(), MISSING_ID), teacherAlgDs.getId());
	...
	void deleteByIds_nullOrEmpty_returnsEmpty() {
		var r1 = lessonService.deleteByIds(null, teacherAlgDs.getId());
		var r2 = lessonService.deleteByIds(List.of(), teacherAlgDs.getId());
	...
	void deleteByIds_notOwned_fails() {
		assertThrows(IllegalStateException.class,
				() -> lessonService.deleteByIds(List.of(base.getId()), teacherNet.getId()));
	}
}
